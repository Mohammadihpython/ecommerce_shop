...
def test_inventory_db_product_inventory_database(
    db,
    db_fixture_setup,
    id,
    sku,
...
def test_inventory_db_product_insert_data(
    ...
    assert new_product.brand.name == "new_brand"
...
def test_inventory_db_producttype_uniqueness_integrity(
    ...
    product_type_factory.create(name="not_uniqe")
    with pytest.raises(IntegrityError):
        product_type_factory.create(name="not_uniqe")
...
def test_inventory_db_brand_uniqeness_integrity(db, brand_factory):
    brand_factory.create(name="not_brand_uniqe")
    with pytest.raises(IntegrityError):
        brand_factory.create(name="not_brand_uniqe")
...
def test_inventory_db_stock_dataset(
    ...
    result_last_check = result.last_checked.strftime("%Y-%m-%d %H:%M:%S")
    assert result.product_inventory.id == product_inventory
    assert result_last_check == last_checked
...
def test_inventory_db_product_attribute_attribute_integrity(
    db,product_attribute_factory
):
...
def test_inventory_db_product_attribute_insert_data(
    ...
    new_attribute_value =  product_attribute_value_factory.create(
        attribute_value="new_value",product_attribute__name="new_name"
    )
    assert new_attribute_value.attribute_value == "new_value"
    assert new_attribute_value.product_attribute.name == "new_name"
    
    
def test_inventory_db_insert_inventory_product_values(
    db,product_with_attribute_values_factory,
):
    new_inv_attribute_values = product_with_attribute_values_factory.create(sku="123456789")   
